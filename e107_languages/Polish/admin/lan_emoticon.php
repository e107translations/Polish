<?php
/*
+====================================================================+
+--------------------------------------------------------------------+
   * Spolszczenie systemu CMS e107                                   *
   * Polish language file - admin/lan_emoticon.php                   *
	* $Date: 2019/08/14                                              *
	* $Author: JacekCich & akogo 									 *
+--------------------------------------------------------------------+
+====================================================================+
*/

define("EMOLAN_1", "Aktywacja emotikon");
//define("EMOLAN_2", "Name");
define("EMOLAN_3", "Emotikony");
define("EMOLAN_4", "Aktywować emotikony?");

define("EMOLAN_5", "Grafika");
define("EMOLAN_6", "Kod grafiki");
define("EMOLAN_7", "do oddzielenia więcej niż jednego wpisu stosuj spację");

//define("EMOLAN_8", "Status");
//define("EMOLAN_9", "Options");
//define("EMOLAN_10", "Active");
define("EMOLAN_11", "Aktywuj zestaw");

//define("EMOLAN_12", "Edit / configure this pack");
define("EMOLAN_13", "Zainstalowane zestawy");

//define("EMOLAN_14", "Save configuration");
//define("EMOLAN_15", "Edit / configure emotes");
//define("EMOLAN_16", "Emote configuration saved");
define("EMOLAN_17", "Obecnie posiadasz zestaw emotikon, który zawiera w nazwie niedozwolone odstępy!");
define("EMOLAN_18", "Proszę zmienić nazwę wymienionych poniżej elementów tak, aby nie zawierały w nazwie odstępów");
//define("EMOLAN_19", "Name");
define("EMOLAN_20", "Lokalizacja");
define("EMOLAN_21", "Błąd");

define("EMOLAN_22", "Znaleziono nowy zestaw emotikon");
define("EMOLAN_23", "Znaleziono nowy zestaw (xml) emotikon");
define("EMOLAN_24", "Znaleziono nowy zestaw (php) emotikon");
//define("EMOLAN_25", "Installing new PHP emotes");
define("EMOLAN_26", "Odśwież zestaw");
define("EMOLAN_27", "Wystąpił błąd podczas odczytu zestawu");
define("EMOLAN_28", "Wygeneruj XML");
define("EMOLAN_29", "Wygenerowano plik XML");
define("EMOLAN_30", "Błąd zapisu pliku XML");

// 2.x
define("EMOLAN_PAGE_TITLE", " Emotikony");

define("EMOLAN_31", "Łącznie znalezionych [x] plików");
define("EMOLAN_32", "Wykryto nieznany zestaw");
define("EMOLAN_33", "Niewspierany format xml pliku");
define("EMOLAN_34", "Brakujące pliki dla zestawu");
define("EMOLAN_35", " - skasowano w bazie danych");
define("EMOLAN_37", "Emotikony nie ustawione");
define("EMOLAN_38", "Pusta wartość emotikon");
?>